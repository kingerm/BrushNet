...
class BoundedAttention(injection_utils.AttentionBase):
    ...
    def reset(self):
        ...
        super().reset()
    # 就相当于在migcprocessor里面，再加一个判定条件：basteps>25，然后分cross-attention和self-attention两种进行处理
    def forward(self, q, k, v, is_cross, place_in_unet, num_heads, **kwargs):  # q: (16, 256, 160)
        ...
        sim = torch.einsum('b i d, b j d -> b i j', q, k) * kwargs['scale']  # 这是做scale-dot production
        attn = sim.softmax(-1)
        ...
        return einops.rearrange(out, '(b h) n d -> b n (h d)', h=num_heads)  # 这里搞完，就进入injection_utils里面的AttentionBase的__call__函数
        # 这个值正是hiddenstates，两者完全可以缝合在一起！太棒了！size: (2, 4096, 320)，和migcprocessor里面的hiddenstates的size一模一样！
...
# obtain_masks有点像是refine mask的函数哦
    def _obtain_masks(self, resolution, return_boxes=False, return_existing=False, batch_size=None, device=None):
        ...
        if return_boxes or self.cur_step < self.min_clustering_step:
            ...
            masks = self._obtain_self_masks(resolution, return_existing=return_existing)  # 获得聚类后的mask后再返回
...
# build_self_masks更像是生成sa mask的函数！
    ...
    def _cluster_self_maps(self):  # b s n
        self_maps = self._compute_maps(self.mean_self_map)  # b n m  # self.mean_self_map初始化确实是int: 0
