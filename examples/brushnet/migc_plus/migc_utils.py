...
def load_migc(unet, attention_store, pretrained_MIGC_path: Union[str, Dict[str, torch.Tensor]], attn_processor, strict,
                        **kwargs):
    ...
    for key in all_processor_keys:
        if key not in attn_processors.keys():
            if key.startswith("mid_block"):
                ...
                place_in_unet = "down"
            attn_processors[key] = attn_processor(config, attention_store, place_in_unet, migc_type='migc')
    unet.set_attn_processor(attn_processors)
    attention_store.num_att_layers = 32


def load_migc_plus(unet, attention_store, pretrained_MIGC_path: Union[str, Dict[str, torch.Tensor]], attn_processor, strict=False,
                        **kwargs):
    ...
    for key, value_dict in adapter_grouped_dict.items():
        ...
        attn_processors[key_final] = attn_processor(config, attention_store, place_in_unet, migc_type='migc++')
        attn_processors[key_final].load_state_dict(value_dict, strict=strict)
        attn_processors[key_final].to(device=unet.device, dtype=unet.dtype)
    ...
    for key in all_processor_keys:
        if key not in attn_processors.keys():
            if key.startswith("mid_block"):
                ...
                place_in_unet = "down"
            attn_processors[key] = attn_processor(config, attention_store, place_in_unet, migc_type='no_migc')
    unet.set_attn_processor(attn_processors)
    attention_store.num_att_layers = 32


def load_migc_plus_depth(unet, attention_store, pretrained_MIGC_path: Union[str, Dict[str, torch.Tensor]], attn_processor, strict=False,
                        **kwargs):
    ...
    for key, value_dict in adapter_grouped_dict.items():
        ...
        attn_processors[key_final] = attn_processor(config, attention_store, place_in_unet, migc_type='migc++_depth')
        attn_processors[key_final].load_state_dict(value_dict, strict=strict)
        attn_processors[key_final].to(device=unet.device, dtype=unet.dtype)
...
def load_migc_plus_depth_clip(unet, attention_store, pretrained_MIGC_path: Union[str, Dict[str, torch.Tensor]], attn_processor, strict=False,
                        **kwargs):
    ...
    for key, value_dict in adapter_grouped_dict.items():
        ...
        attn_processors[key_final] = attn_processor(config, attention_store, place_in_unet, migc_type='migc++_depth_clip')
        attn_processors[key_final].load_state_dict(value_dict, strict=strict)
        attn_processors[key_final].to(device=unet.device, dtype=unet.dtype)
