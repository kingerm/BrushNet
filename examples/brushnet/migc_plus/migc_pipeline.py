...
import cv2
import math
from migc_plus.migc_arch import MIGC, MIGC_plus, MIGC_plus_depth, MIGC_plus_depth_clip, NaiveFuser
...
MIGC_book = {'migc': MIGC, 'migc++': MIGC_plus, 'migc++_depth': MIGC_plus_depth, 'migc++_depth_clip': MIGC_plus_depth_clip}

class MIGCProcessor(nn.Module):
    def __init__(self, config, attnstore, place_in_unet, migc_type='migc'):
        super().__init__()
        ...
        self.embedding = {}
        self.is_migc_plus = (migc_type == 'migc++' or migc_type == 'migc++_depth' or migc_type == 'migc++_depth_clip')
        if not self.not_use_migc:
            self.migc = MIGC_book[migc_type](config['C'])
    ...
    def __call__(
        ...
        if masks is None:
            ...
            assert self.is_migc_plus or self.not_use_migc, "MIGC only supports box guidance, please use MIGC++ for mask guidance."
        ...
        supplement_mask = torch.from_numpy(sup_mask[None, ...])  # (1, 1 ,896, 640)  # 这弄出来的是uint8，无法用interpolate
        # supplement_mask = F.interpolate(supplement_mask, (height//8, width//8), mode='bilinear').float()
        supplement_mask = F.interpolate(supplement_mask.float(), (height // 8, width // 8), mode='bilinear').half()
        ...
        in_box = torch.from_numpy(np.array(in_box))[None, ...].half().to(cond_ca_output.device)  # (1, instance_num, 4)
        
        if not depth is None:
            in_depth = torch.from_numpy(np.array(depth[0]))[None, ...].half().to(cond_ca_output.device)  # (1, instance_num, 1)
        else:
            in_depth = None

        if not depth_img_token is None:
            depth_img_token = depth_img_token.to(cond_ca_output.device)
        else:
            depth_img_token = None
        ...
        other_info['instance_img'] = depth_img_token
