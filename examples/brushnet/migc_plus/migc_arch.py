...
import math
from migc_plus.migc_layers import CBAM, CrossAttention, LayoutAttention, CrossAttention_depth
...
class DepthNet(nn.Module):
    def __init__(self, in_dim, out_dim, fourier_freqs=8):
        ...
        self.fourier_embedder = FourierEmbedder(num_freqs=fourier_freqs)
        self.position_dim = fourier_freqs * 2 * 4  # 2 is sin&cos
    ...
    def forward(self, depth):
        # embedding depth.        
        depth = depth.repeat(1, 1, 4)  # B*1*1 --> B*1*4 torch.Size([5, 1, 4])

        depth_embedding = self.fourier_embedder(depth)  # B*1*1 --> B*1*C torch.Size([5, 1, 64])
        depth_embedding = self.linears_position(depth_embedding)  # B*1*C --> B*1*768 torch.Size([5, 1, 768])
...
class NaiveFuser(nn.Module):
    ...
    def forward(self, ca_x, guidance_mask, other_info, ca_scale=None, return_fuser_info=False):
        ...
        W = full_W // down_scale
        guidance_mask = F.interpolate(guidance_mask, size=(H, W), mode='bilinear')   # (B, instance_num, H, W)
        if other_info['depth'] is None:
            guidance_mask = torch.cat([torch.ones(B, 1, H, W).to(guidance_mask.device), guidance_mask * other_info.get('refined_alpha', 10.0)], dim=1)  # (B, instance_num+1, H, W)
        else:
            depth = other_info['depth']
            # print(depth.shape)
            assert depth.shape == (B, instance_num, 1)
            # depth = depth.view(B, instance_num)
            for i in range(instance_num):
                guidance_mask[:, i, ...] = guidance_mask[:, i, ...] * math.exp(1.0 / (0.4 + depth[:, i, ...]))
...
class SAC_plus(nn.Module):
    ...
    def forward(self, x, guidance_mask=None, sac_scale=None, H=0, W=0):
        '''
        :param x: (B, phase_num, HW, C)
        :param guidance_mask: (B, phase_num, H, W)
        :return:
        ...
        B, phase_num, HW, C = x.shape
        if math.sqrt(HW) % 1 == 0:
            H = W = int(math.sqrt(HW))  # 这一行不对，不能保证HW被整除
        null_x = torch.zeros_like(x[:, [0], ...]).to(x.device)
...
class MIGC_plus(nn.Module):
    ...
    def forward(self, ca_x, guidance_mask, other_info, return_fuser_info=False):
        ...
        ca_scale = other_info['ca_scale'] if 'ca_scale' in other_info else None
        ea_scale = other_info['ea_scale'] if 'ea_scale' in other_info else None
        mig_ca_x = self.naive_fuser(ca_x, guidance_mask, other_info, ca_scale=ca_scale)  # (B, 1, HW, C)
        full_H = other_info['height']
        full_W = other_info['width']
        B, _, HW, C = ca_x.shape
        instance_num = guidance_mask.shape[1]
        ...
        out_MIGC, sac_scale = self.sac(shading_instances_and_template, sac_scale=sac_scale, H=H, W=W)  # (B, 1, HW, C)
        out_MIGC = mig_ca_x + out_MIGC * torch.tanh(self.gamma)
        if return_fuser_info:
            fuser_info = {}
...
# class MIGC_plus_depth(nn.Module):
#     def __init__(self, C, attn_type='base', context_dim=768, heads=8):
#         super().__init__()
#         self.ea = CrossAttention(query_dim=C, context_dim=context_dim,
#                              heads=heads, dim_head=C // heads,
...
#         self.sac = SAC_plus(C)
#         self.pos_net = PositionNet(in_dim=768, out_dim=768)
#         self.depth_net = DepthNet(in_dim=768, out_dim=768)
#         self.gamma = nn.Parameter(torch.zeros(1))
#         self.naive_fuser = NaiveFuser()
...
#         # box: (B, instance_num, 4)
#         # image_token: (B, instance_num+1, HW, C)
#         mig_ca_x = self.naive_fuser(ca_x, guidance_mask, other_info)  # (B, 1, HW, C)
#         full_H = other_info['height']
#         full_W = other_info['width']
#         B, _, HW, C = ca_x.shape
#         instance_num = guidance_mask.shape[1]
...
#         box = box.reshape(B * instance_num, 1, -1)
#         depth = other_info['depth']
#         depth = depth.reshape(B * instance_num, 1, -1)
#         # print(f"box:{box.device}, depth:{depth.device}")
#         box_token = self.pos_net(box)
#         depth_token = self.depth_net(depth)
#         if context.ndim == 3:
#             context = torch.cat([context[1:, ...].reshape(B * instance_num, 1, -1), box_token, depth_token], dim=1)
#         else:
#             context = torch.cat([context[:, 1:, ...].reshape(B * instance_num, 1, -1), box_token, depth_token], dim=1)
#         ca_scale = other_info['ca_scale'] if 'ca_scale' in other_info else None
#         ea_scale = other_info['ea_scale'] if 'ea_scale' in other_info else None
...
#         #         ca_x[:, i+1, ...] = ca_x[:, i+1, ...] * ca_scale[i] + ea_x[:, i, ...] * ea_scale[i]
#         # else:
#         #     ca_x[:, 1:, ...] = ca_x[:, 1:, ...] + ea_x * torch.tanh(self.gamma)
...
#             fuser_info = {}
#             fuser_info['sac_scale'] = sac_scale.view(B, instance_num + 1, H, W)
#             fuser_info['ea_attn'] = ea_attn.mean(dim=1).view(B, instance_num, H, W, 3)
#             return out_MIGC, fuser_info
#         else:
#             return out_MIGC
...
class MIGC_plus_depth(nn.Module):
    def __init__(self, C, attn_type='base', context_dim=768, heads=8):
        super().__init__()
        self.ea = CrossAttention_depth(query_dim=C, context_dim=context_dim,
                             heads=heads, dim_head=C // heads,
                             dropout=0.0)
        self.la = LayoutAttention(query_dim=C,
                                    heads=heads, dim_head=C // heads,
                                    dropout=0.0)
        self.norm = nn.LayerNorm(C)
        # self.depth_linear = nn.Linear(768, 768)
        self.sac = SAC_plus(C)
        self.pos_net = PositionNet(in_dim=768, out_dim=768)
        self.depth_net = DepthNet(in_dim=768, out_dim=768)
        self.gamma = nn.Parameter(torch.zeros(1))
    ...
    def forward(self, ca_x, guidance_mask, other_info, return_fuser_info=False):
        ...
        box_token = self.pos_net(box)
        depth_token = self.depth_net(depth)
        # depth_token = self.depth_linear(depth_token)
        if context.ndim == 3:
            context = torch.cat([context[1:, ...].reshape(B * instance_num, 1, -1), box_token], dim=1)
        else:
            context = torch.cat([context[:, 1:, ...].reshape(B * instance_num, 1, -1), box_token], dim=1)
        ...
        ea_x, ea_attn = self.ea(self.norm(image_token[:, 1:, ...].reshape(B * instance_num, HW, C)),
                                     context=context, context_depth=depth_token, return_attn=True)
        ea_x = ea_x.view(B, instance_num, HW, C)
        ea_x = ea_x * guidance_mask.view(B, instance_num, HW, 1)
...
class MIGC_plus_depth_clip(nn.Module):
    def __init__(self, C, attn_type='base', context_dim=768, heads=8):
        super().__init__()
        self.ea = CrossAttention(query_dim=C, context_dim=context_dim,
                             heads=heads, dim_head=C // heads,
    ...
    def forward(self, ca_x, guidance_mask, other_info, return_fuser_info=False):
        ...
        # depth = other_info['depth']        
        # depth = depth.reshape(B * instance_num, 1, -1)
        instance_img = other_info['instance_img']
        instance_img = instance_img.reshape(B * instance_num, 257, 1024)
        instance_img = self.img_linear(instance_img)
        # print(f"box:{box.device}, depth:{depth.device}")
        box_token = self.pos_net(box)
        if context.ndim == 3:
            context = torch.cat([context[1:, ...].reshape(B * instance_num, 1, -1), box_token, instance_img], dim=1)
        else:
            context = torch.cat([context[:, 1:, ...].reshape(B * instance_num, 1, -1), box_token, instance_img], dim=1)
        ca_scale = other_info['ca_scale'] if 'ca_scale' in other_info else None
        ea_scale = other_info['ea_scale'] if 'ea_scale' in other_info else None
