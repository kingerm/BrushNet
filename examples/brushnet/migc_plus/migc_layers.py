...
class CrossAttention_depth(nn.Module):
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.):
        super().__init__()
        inner_dim = dim_head * heads
        context_dim = default(context_dim, query_dim)
        ...
        self.to_k = nn.Linear(context_dim, inner_dim, bias=False)
        self.to_v = nn.Linear(context_dim, inner_dim, bias=False)
        self.to_k_depth = nn.Linear(context_dim, inner_dim, bias=False)
        self.to_v_depth = nn.Linear(context_dim, inner_dim, bias=False)
    ...
    def forward(self, x, context=None, context_depth=None, mask=None, return_attn=False, need_softmax=True, guidance_mask=None,
                forward_layout_guidance=False):
        h = self.heads
        b = x.shape[0]
        ...
        k = self.to_k(context)
        v = self.to_v(context)
        k_depth = self.to_k_depth(context_depth)
        v_depth = self.to_v_depth(context_depth)
        k = torch.cat([k, k_depth], dim=1)
        v = torch.cat([v, v_depth], dim=1)
