...
class ImageReconstruction:
    ...
    def reconstruct_image_by_optimization(
        ...
        # 上面两行代码对应self.vae.encode(image).latent_dist.sample() * self.vae.config.scaling_factor
        if self.opt.optimization_mode == "weights":
            curr_latent = initial_latent.clone().detach()
            decoder_copy = copy.deepcopy(self.model.first_stage_model.decoder)  # 对应self.vae.decode
